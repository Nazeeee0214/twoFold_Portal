...
try {
    // Total records count
    $totalQuery = $pdo->query("SELECT COUNT(*) FROM users WHERE restriction = 'ADMIN'");
    $totalRecords = $totalQuery->fetchColumn();

    // Fetch paginated data
    $stmt = $pdo->prepare("SELECT user_id, fullname, status FROM users WHERE restriction = 'ADMIN' LIMIT :start, :length");
